...
An enemy Task Force moves into a Sea Zone where you have a submarine On Patrol. The on-patrol sub can interrupt and attack the moving TF (5.2.3). The attack uses the standard 7.3 sub-vs-TF torpedo procedure. Unlike reacting against a moving submarine, this flips the reacting sub to Spent (5.2.3).
...
7. Flip the On-Patrol sub to Spent (5.2.3). Reacting against a moving TF, unlike reacting against a moving submarine, ends the sub's on-patrol duty for the turn.
...
This reaction is free in OPS but flips the sub to Spent after firing (5.2.3), unlike reacting against a moving submarine, which leaves it On Patrol. Only one On-Patrol unit per zone may react against a given moving unit (5.2.3), so hold fire on a screening Convoy TF if a carrier TF is coming next, since this is your one shot for the turn. TF still gets its full ASW defense roll with Fast and Bad Weather row shifts (7.3.1).
...
A NATO Trafalgar On Patrol in Mid 7-8 watches a Good-detected Soviet TF enter. Anti-Surface 3 +1 (Good) = 4 dice, scores 2 hits on 7s. Soviet TF ASW total 10 = 3 dice + 1 Tactical, rolls 9, 6, 5, discards one hit on the 9, the other hit lands as a step loss on a Sovremenny. The Trafalgar flips to Spent, its patrol duty for the turn done.
